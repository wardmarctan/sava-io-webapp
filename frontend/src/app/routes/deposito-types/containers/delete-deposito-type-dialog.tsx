...
import { deleteDepositoType } from '@/lib/api/deposito-types/delete-deposito-type'
import type { DepositoType } from '@/lib/api/types/deposito-type/deposito-type'
import { useTranslation, Trans } from 'react-i18next'
import { toast } from 'sonner'

interface DeleteDepositoTypeDialogProps {
  open: boolean
...
export function DeleteDepositoTypeDialog({
  open,
  onOpenChange,
  depositoType,
  label,
  onDeleted,
}: Readonly<DeleteDepositoTypeDialogProps>) {
  ...
  const onDelete = async () => {
    ...
    try {
      await deleteDepositoType(depositoType.id)
      toast.success(t('toast.deleteSuccess'))
      await onDeleted()
      onOpenChange(false)
    } catch (deleteError) {
      const errMsg = deleteError instanceof Error ? deleteError.message : 'Failed to delete deposito type'
      setError(errMsg)
  ...
  if (!open || !depositoType) return null
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="space-y-3">
          ...
          <p className="text-sm text-slate-600">
            <Trans i18nKey="delete.confirm" ns="depositoTypes" values={{ label }}>
              Are you sure you want to delete <span className="font-semibold text-slate-950">{label}</span>?
            </Trans>
          </p>
        </div>
